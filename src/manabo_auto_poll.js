// ==UserScript==
// @name         中京大学MaNaBo アンケート自動回答
// @description  アンケートに自動回答し、カスタム確認ダイアログを含めて最後まで自動で送信します。
// @match        https://manabo.cnc.chukyo-u.ac.jp/addon/survey/class/attend/*
...
(function() {
    ...
    function answerQuestions() {
        try {
            ...
            questions.forEach(row => {
                ...
                const textarea = row.querySelector('textarea.answer-form');
    ...
    function submitAndConfirm() {
        ...
        const dialogObserver = new MutationObserver((mutations, obs) => {
            ...
            const confirmYesButton = document.querySelector('.btn-common-confirm-modal-yes');
            if (confirmYesButton && confirmYesButton.offsetParent !== null) {
                confirmYesButton.click();
            }

            // 2. 最終の完了ダイアログを探す (「OK」をクリック)
            const finalOkButton = document.querySelector("#btn-common-alert-modal-ok");
        ...
        setTimeout(() => {
            const submitButton = document.querySelector(".attend-finish-button");
            if (submitButton) {
