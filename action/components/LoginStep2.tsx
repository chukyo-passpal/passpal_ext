...
const LoginStep2: React.FC<LoginStep2Props> = ({
  ...
  const signInWithGoogle = async (): Promise<void> => {
    try {
      ...
      const authData = await new Promise<AuthResponse | FirebaseError>(
        (resolve, reject) => {
      ...
      if (authData && "name" in authData && authData.name === "FirebaseError") {
        ...
        // ドメインエラーのチェック
        if (
          error.code === "auth/invalid-domain" ||
          (error.message && error.message.includes("chukyo-u.ac.jp"))
        ) {
    ...
    <div className="login-step">
      ...
      <div className="login-form">
        ...
        <button
          ...
          disabled={isLoading}
          className={`login-button google ${isLoading ? "loading" : ""}`}
        >
