...
const ICONS = {
  ...
  const setFocusedVideo = (newVideo) => {
  ...
  const highlightButtonByKey = (videoWrapper, key) => {
    if (!videoWrapper) return;
    const button = videoWrapper.querySelector(`[data-vctrl="${key}"]`);
    if (button) {
      if (button.highlightTimeout) clearTimeout(button.highlightTimeout);
  ...
  // ★追加: フィードバック表示用の関数
  const showFeedback = (videoWrapper, icon, text = "") => {
    if (!videoWrapper) return;
  ...
  document.addEventListener("fullscreenchange", () => {
    ...
    if (fsElement) {
      if (typeof fsElement.focus === "function") {
        fsElement.focus({ preventScroll: true });
      }
    } else {
  ...
  function initializeVideoControls(video) {
    ...
    const createUIContainer = (className, elements) => {
    ...
    const setPlaybackRate = (rate) => {
      video.playbackRate = clamp(parseFloat(rate) || 1.0, MIN_RATE, MAX_RATE);
    };
    ...
    const speedControls = createUIContainer("v-ctrl-speed", [
      decreaseBtn,
      speedDisplay,
      increaseBtn,
    ]);
    ...
    if (document.pictureInPictureEnabled) {
      ...
      const updatePipButton = () => {
        ...
        pipButton.textContent = isPiP ? "PiP終了" : "PiP";
    ...
    (e) => {
      ...
      if (focusedVideo && document.contains(focusedVideo)) {
        ...
        const allVideos = Array.from(
          document.querySelectorAll(`video[${INITIALIZED_ATTR}]`)
        );
        const fsElement = document.fullscreenElement;
        if (fsElement) {
          targetVideo = fsElement.matches("video")
            ? fsElement
            : fsElement.querySelector("video");
        }
        if (!targetVideo) {
          targetVideo = allVideos.find((v) => !v.paused);
        }
      ...
      let handled = true;
      const setRate = (rate) =>
        (targetVideo.playbackRate = clamp(rate, MIN_RATE, MAX_RATE));
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "arrowup":
          ...
          const upIcon =
            targetVideo.volume >= 0.5 ? ICONS.volumeUp : ICONS.volumeDown;
          showFeedback(
            wrap,
            targetVideo.volume === 0 ? ICONS.volumeMute : upIcon,
            `${Math.round(targetVideo.volume * 100)}%`
          );
          break;
        case "arrowdown":
          targetVideo.volume = clamp(targetVideo.volume - 0.1, 0, 1);
          ...
          showFeedback(
            wrap,
            targetVideo.volume === 0 ? ICONS.volumeMute : downIcon,
            `${Math.round(targetVideo.volume * 100)}%`
          );
          break;
        case "m":
          ...
          if (targetVideo.muted) {
            ...
            showFeedback(
              wrap,
              targetVideo.volume === 0 ? ICONS.volumeMute : currentVolumeIcon,
              `${Math.round(targetVideo.volume * 100)}%`
            );
          }
          break;
      ...
      if (mutation.type === "childList") {
        for (const node of mutation.addedNodes) {
          ...
          const videos = node.matches("video")
            ? [node]
            : node.querySelectorAll(`video:not([${INITIALIZED_ATTR}])`);
          videos.forEach(setupVideo);
        }
      }
      if (
        mutation.type === "attributes" &&
        mutation.attributeName === INITIALIZED_ATTR
      ) {
        const video = mutation.target;
        if (
          video.matches("video") &&
          video.getAttribute(INITIALIZED_ATTR) === "true"
        ) {
          setFocusedVideo(video);
        }
      }
  ...
  document
    .querySelectorAll(`video:not([${INITIALIZED_ATTR}])`)
    .forEach(setupVideo);
})();
